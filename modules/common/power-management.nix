...
in
{
  options.myConfig.apps.power.management = {
    ...
    backend = lib.mkOption {
      type    = lib.types.enum [ "power-profiles-daemon" "tuned" "tlp" ];
  ...
  config = lib.mkIf cfg.services {
    services.power-profiles-daemon.enable = cfg.backend == "power-profiles-daemon";
    services.tuned.enable                 = cfg.backend == "tuned";
    services.tlp.enable                   = cfg.backend == "tlp";
  };
}