...
- Role-based composition (`minimal`, `bootstrap`, `server`, `desktop`, `workstation`, `family`)
...
Hosts with a defined `ip` field are automatically included in Colmena and deploy-rs targets, only hosts with `enabled = true` are considered in flake outputs.
...
**3. Add it to `.sops.yaml`** (a new anchor under `keys`, and in every rule the host must read; if the host is already listed from a previous install, **replace** its old key):
  ...
  - path_regex: secrets/common\.yaml$
    key_groups:
      - age:
          - *admin_bensuperpc
          - *server_1_m710q
  - path_regex: secrets/hosts/server-1-m710q\.yaml$
    key_groups:
      - age:
          - *admin_bensuperpc
          - *server_1_m710q
...
**4. Re-encrypt the secrets for the new recipient** (`common.yaml` holds the `bensuperpc` password, so every host needs it; the per-host file only exists if the host uses one, e.g. the MicroVM example):

```bash
sops updatekeys -y secrets/common.yaml
sops updatekeys -y secrets/hosts/$HOST.yaml
```

Check that the new recipient is listed, and that the admin key still decrypts:

```bash
grep 'recipient:' secrets/common.yaml
sops -d secrets/common.yaml > /dev/null && echo OK
```

**5. switch the host to its real role** (e.g. `full`, `desktop`, etc.) in `systems/<host>/definition.nix`, update configuration and reboot.
...
| `bootstrap`   | `platform/base`, `platform/bootstrap`                                       | _(none)_                                                                                                                                                                                                                                                                             | _(none)_                |
...
```nix
{
  ...
  role = "desktop";           # minimal | bootstrap | server | wsl | desktop | workstation | full | family
  system = "x86_64-linux";
